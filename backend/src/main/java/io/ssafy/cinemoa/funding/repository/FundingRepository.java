...
public interface FundingRepository extends JpaRepository<Funding, Long> {
    ...
    /**
     * 특정 날짜에 마감되고 특정 상태인 펀딩들을 Cinema와 함께 조회합니다. N+1 문제를 방지하기 위해 JOIN FETCH를 사용합니다.
     *
     * @param endsOn 마감일
     * @param state  펀딩 상태
    ...
    /**
     * 특정 날짜에 마감되고 특정 상태인 펀딩들을 Cinema와 Screen과 함께 조회합니다. N+1 문제를 방지하기 위해 JOIN FETCH를 사용합니다.
     *
     * @param endsOn 마감일
     * @param state  펀딩 상태
    ...
    List<Funding> findByEndsOnAndStateWithCinemaAndScreen(@Param("endsOn") LocalDate endsOn,
                                                          @Param("state") FundingState state);


    @Query("""
            SELECT new io.ssafy.cinemoa.image.dto.AnimateTask(f.id, f.bannerUrl)
            FROM Funding f
            WHERE f.state = 'SUCCESS' AND f.ticketBanner IS NULL
            """)
    List<AnimateTask> findAnimateRequired();
}
