...
import io.ssafy.cinemoa.funding.dto.LikedFundingListResponseDto;
...
public class LikedFundingService {
    ...
    /**
     * 특정 사용자가 보고싶어요한 펀딩/투표 목록을 조회합니다.
    ...
    public LikedFundingListResponseDto getLikedFundings(Long userId, String typeStr, Long cursor, Integer limit) {
        // 1. 사용자 존재 여부 확인
        ...
        LikedFundingRequestDto request = new LikedFundingRequestDto(null, cursor, limit + 1);

        // 4. 펀딩 데이터 조회 (무한 스크롤)
        Page<LikedFundingItemDto> fundingPage = likedFundingRepository.findLikedFundings(userId, request, typeStr);

        // 5. 펀딩 목록 추출
        List<LikedFundingItemDto> content = fundingPage.getContent();
        ...
        // 8. 빈 데이터인 경우 처리
        if (content.isEmpty()) {
            return LikedFundingListResponseDto.builder()
                    .content(List.of())
     ...
     * 파라미터 검증 및 기본값 설정
     *
     * @param type 펀딩/투표 타입 필터
     * @param cursor 커서 값
     * @param limit 조회 개수
     * @return 검증된 limit 값
     */
    private Integer validateAndSetDefaults(FundingType type, Long cursor, Integer limit) {
     ...
     * @return 페이지 정보 DTO
     */
    private PageInfoDto createPageInfo(Page<LikedFundingItemDto> page) {
     ...
     */
    private PaginationDto createPaginationDto(List<LikedFundingItemDto> content, Integer limit, Long totalCount) {
