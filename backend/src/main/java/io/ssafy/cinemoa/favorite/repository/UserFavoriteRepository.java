...
public interface UserFavoriteRepository extends JpaRepository<UserFavorite, UserFavoriteId> {
    ...
    /**
     * 펀딩 ID로 좋아요를 누른 사용자 ID 목록 조회
     */
    @Query("SELECT uf.user.id FROM UserFavorite uf WHERE uf.funding.fundingId = :fundingId")
    List<Long> findUserIdsByFunding_FundingId(@Param("fundingId") Long fundingId);
