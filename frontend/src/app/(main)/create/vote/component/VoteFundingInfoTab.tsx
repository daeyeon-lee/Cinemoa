...
const voteFormSchema = z.object({
  title: z.string().min(1, '수요조사 제목을 입력해주세요'),
  content: z.string().min(1, '수요조사 내용을 입력해주세요'),
});
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 ">
                ...
                <div className="flex-1">
                  <FormControl>
                    <Input placeholder="수요조사 제목을 입력해주세요" {...field} className="w-full" />
                  </FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-4">
                ...
                <div className="flex-1">
                  <FormControl>
                    <Textarea placeholder="생성할 수요조사에 대해 자세히 설명해주세요" {...field} className="min-h-[135px] resize-none" />
                  </FormControl>
                  <FormMessage className="mt-1" />
                </div>
              </FormItem>
