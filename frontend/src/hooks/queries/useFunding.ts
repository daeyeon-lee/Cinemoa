...
export function useFundingLike() {
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      const existingMutation = queryClient.getMutationCache().find({
        predicate: (mutation) => {
          ...
          return state.status === 'pending' && mutation.options.mutationKey?.[0] === 'fundingLike' && mutation.options.mutationKey?.[1] === fundingId && mutation.options.mutationKey?.[2] === userId;
        },
      });
    ...
    onMutate: async ({ fundingId, userId, isLiked }) => {
      ...
      // 상세 페이지 캐시 업데이트
      queryClient.setQueryData(['DETAIL', fundingId.toString(), userId], (old: ApiResponse<DetailData> | undefined) => {
