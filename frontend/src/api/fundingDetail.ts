...
export const getFundingDetail = async (fundingId: string, userId?: string): Promise<ApiResponse<DetailData>> => {
  try {
    const searchParams = new URLSearchParams();
    if (userId) searchParams.set('userId', userId);

    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}funding/${fundingId}?${searchParams.toString()}`, {
