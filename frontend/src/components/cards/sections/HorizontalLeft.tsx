      ...
      <div className="flex-1 space-y-2">
        ...
        <div className="flex flex-wrap gap-1">
          <span className="px-2 py-1 bg-slate-600 text-slate-300 text-xs rounded">
            {formatLocation ? formatLocation(data.cinema.city, data.cinema.district) : `${data.cinema.city} ${data.cinema.district}`}
          </span>
          <span className="px-2 py-1 bg-slate-600 text-slate-300 text-xs rounded">
        ...
        <div className="text-xs text-slate-400">
          {isFunding ? (
            <span>{data.funding.participantCount}/{data.funding.maxPeople}명 참여 • {data.funding.progressRate}%</span>
          ) : (
