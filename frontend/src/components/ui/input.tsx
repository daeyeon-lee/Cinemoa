...
const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<'input'>>(
  ({ className, type, ...props }, ref) => {
    ...
    const getTypeClasses = (inputType?: string) => {
      switch (inputType) {
        default:
          return 'flex h-10 w-full rounded-[6px] bg-BG-1 px-3 py-1 text-p2-b shadow-sm transition-colors placeholder:text-subtle placeholder:text-p2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm';
        case 'checkbox':
