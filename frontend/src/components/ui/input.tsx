  ...
  ({ className, type, ...props }, ref) => {
    ...
    const getTypeClasses = (inputType?: string) => {
      switch (inputType) {
        ...
        default:
          return 'flex h-10 w-full rounded-[6px] bg-BG-1 px-3 py-1 text-p2-b shadow-sm transition-colors placeholder:text-subtle placeholder:p1-b focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm';
        case 'number':
